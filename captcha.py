...
class reCAPTCHA:
    @staticmethod
    def bypass(token) -> str:
        captcha_url = "https://www.google.com/recaptcha/enterprise/reload?k=6LezjdAZAAAAAD1FaW81QpkkplPNzCNnIOU5anHw"
        headers = {
                "Accept": "*/*",
        ...
        req = client.post(captcha_url, headers=headers, data=payload)
        return "".join(re.findall("\[\"rresp\",\"(.*?)\"", str(req.text)))
    
    @staticmethod
    def init():
        req = client.get(token_url, headers={"Accept": "*/*","Pragma": "no-cache","User-Agent": 'Mozilla/5.0 (X11; Linux x86_64; rv:78.0)'})
        return "".join(re.findall("type=\"hidden\" id=\"recaptcha-token\" value=\"(.*?)\"", str(req.text)))
